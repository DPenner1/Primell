...
Operators may only take up to two arguments. Though all operators are able to work on all data types, they are defined in terms of the expected data type, and mechanically generalized when given a different data type. Additionally, operators can be modified to produce variant behaviour. See the [List of Operators](https://github.com/DPenner1/Primell/wiki/List-of-Operators) on the wiki for the full list.
...
Since Primell usually executes in strict left-to-right order, it is quite easy to figure out what happens. Let's go step by step.
 ...
 1. `(2 3 5 7)`: We start with the list (2 3 5). In this case the parentheses are necessary to stop the 5 + 2 from executing first.
 2. `+ 2`: This creates the list `(4 5 7 9)`.
 3. `_~`: Reverses the list giving `(9 7 5 4)`.
 4. `_>`: Gets the tail of the list (all elements after 0th). We now have `(7 5 4)`.
 5. `*;`: Multiplication, modified by `;` to occur across the list, giving the final result of `140`.

Note that are printed to the screen, even with each of these steps run individually. The outermost parentheses are never printed because otherwise it would be possible to distinguish between a 1-element list and its contents (which Primell forbids).

Note that spaces aren't usually necessary, `(2 3 5 7)+2_~_>*;` would have been an identical program.

Programs may have multiple lines and the lines are executed from top to bottom in the same manner. Primell doesn't quite have traditional control-flow like other programming languages, but instead does have the capability to incorporate lines of code within another line. Check out the branch operators in [Conditionals and Control Flow](https://github.com/DPenner1/Primell/wiki/Conditionals-and-Control-Flow). This is probably sufficient for simulating a Turing machine (see the [Examples.md](https://github.com/DPenner1/Primell/blob/main/Examples/Examples.md) file for details).
